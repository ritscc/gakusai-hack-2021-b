...
@Component @RequiredArgsConstructor public class CloudStorageUtil {
    ...
    public String uploadFile(final FileModel file) {
        ...
        try {
            final var storage = StorageOptions.newBuilder().setProjectId(gcpProperty.getProjectId())
              .setCredentials(GoogleCredentials.fromStream(new FileInputStream(this.gcpProperty.getCredentialsPath()))).build()
              .getService();
